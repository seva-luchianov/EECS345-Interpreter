...
(define Mstate
  (lambda (expression state continuations)
    (cond
      [(and (null? expression) (null? (popLayer2 state))) ((getReturnCont continuations) (string-append "Error: No return statement. Final program state: " (string-join (map ~a state) " ")))]
      ...
      [else
       (cond
         ...
         [(eq? (operator expression) 'begin) (with-handlers ([exn:fail? (lambda (exn) ((getReturnCont continuations) "Error: Continue/Break/Throw in an invalid location."))]) (popLayer (Mstate (cdr expression) (addLayer state) (setBreakCont (setContinueCont (setThrowCont continuations (lambda (state3) ((getThrowCont continuations) (popLayer state3)))) (lambda (state3) ((getContinueCont (setThrowCont continuations (lambda (state3) ((getThrowCont continuations) (popLayer state3))))) (popLayer state3)))) (lambda (state2) ((getBreakCont (setContinueCont (setThrowCont continuations (lambda (state3) ((getThrowCont continuations) (popLayer state3)))) (lambda (state3) ((getContinueCont (setThrowCont continuations (lambda (state3) ((getThrowCont continuations) (popLayer state3))))) (popLayer state3))))) (popLayer state2)))))))]
...
(define tryStatement
  (lambda (expression state continuations)
    (cond
      [(findfirst* '%throw% (call/cc (lambda (v) (Mstate (cons 'begin (car expression)) state (setThrowCont continuations v))))) (finallyStatement (finally-expression expression) (catchStatement (car (cdr expression)) (car (call/cc (lambda (v) (Mstate (cons 'begin (car expression)) state (setThrowCont continuations v))))) continuations (cdr (call/cc (lambda (v2) (Mstate (cons 'begin (car expression)) state (setThrowCont continuations v2)))))) continuations)]
...
(define catchStatement
  (lambda (catch-expression state continuations throw-value)
    (cond
      ...
      ;; no throw found
      [(null? throw-value) (Mstate (cons 'begin (car (cdr (cdr catch-expression)))) (Mstate (cons 'var (cons (car (car (cdr catch-expression))) '())) state continuations) continuations)]
      [(Mstate (cons 'begin (car (cdr (cdr catch-expression)))) (Mstate (cons 'var (cons (car (car (cdr catch-expression))) (cons (car throw-value) '()))) state continuations) continuations)])))
...
(define getFinallyCont
  (lambda (lis)
    (car (cdr (cdr (cdr (cdr lis)))))))
...
(define getLookupValue
  (lambda (a state continuations)
    (cond
      [(null? (flatten (LayerLookup a state))) ((getReturnCont continuations) "Error: The variable for lookup not assigned or declared in any layer.")]
...
(define declare
  (lambda (expression state continuations)
    (cond
      ...
      [(not (findfirst* (leftoperand expression) state)) (append (cons (cons (cons (leftoperand expression) '()) (car state)) '()) (cdr state))]
...
(define popLayer
  (lambda (state)
    (cond
      [(null? state) '()]
      [(number? state) state]
      [(findfirst* '%throw% state) (cons (popLayer (car state)) (cdr state))]
